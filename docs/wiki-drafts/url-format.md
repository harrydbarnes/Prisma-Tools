...
Choose a short shareable campaign URL or the full address.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **URL format** and select the matching option. Its default is **short**.
3. Choose Short or Full underneath Auto Copy Campaign URL. The parent option must be enabled. Short uses Prisma’s shareable campaign link; Full uses the current address, including the active route.
...
The option uses Chrome sync storage key `autoCopyUrlMode`. Feature behaviour is implemented in `features/auto-copy-url.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

README summary; Settings preview (338). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:338](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L338)
