...
Adds copy actions for the campaign ID and CL, PR and CA references.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **Campaign header copy** and select the matching option. Its default is **true**.
3. Use the header copy controls for the CP/campaign ID and CL/PR/CA reference. These copy the displayed references rather than navigating away.
...
The option uses Chrome sync storage key `campaignHeaderQuickCopyEnabled`. Feature behaviour is implemented in `features/campaign.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

README summary; Settings preview (332). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:332](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L332)
