...
Adds a one-click copy action for the campaign name.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **Campaign name copy** and select the matching option. Its default is **true**.
3. Click the campaign name copy target in the header and check the confirmation toast. Clipboard errors can occur if the extension has been reloaded while the Prisma tab stayed open; reload that tab and retry.
...
The option uses Chrome sync storage key `campaignNameQuickCopyEnabled`. Feature behaviour is implemented in `features/campaign.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

README summary; Settings preview (331). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:331](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L331)
