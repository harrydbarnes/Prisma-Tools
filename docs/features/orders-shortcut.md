...
Adds an Orders shortcut to the campaign navigation menu.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **Orders shortcut** and select the matching option. Its default is **true**.
3. Use Orders in the campaign navigation to reach the order workflow. Legacy and newer Orders interfaces have different DOM structures; if the control disappears after navigation, reload Prisma and report which interface was active.
...
The option uses Chrome sync storage key `ordersShortcutEnabled`. Feature behaviour is implemented in `features/campaign.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

README summary; Settings preview (327). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:327](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L327)
