# Restore page after account switch

Draft feature guide for Ops Toolshed r1.9. Based on the reviewed source; live Prisma verification is still pending.

## What it does

Returns you to the Prisma page you were viewing after a new account has loaded.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **Restore page after account switch** and select the matching option. Its default is **true**.
3. After switching account, the extension can return to the previously viewed Prisma page. The destination may not be accessible in the new account, so check the account banner before making changes.
...
The option uses Chrome sync storage key `rememberAccountSwitchUrlEnabled`. Feature behaviour is implemented in `features/swap-accounts.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

release notes; Settings preview (342). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:342](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L342)
