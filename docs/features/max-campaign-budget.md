...
3. Use Max Campaign Budget on an editable Buy Cost or active-month Actualise Gross payable cell. It uses the live billable response or a validated projection to close the budget gap. Review the inserted value and resulting total in Prisma, and use Revert change if needed. It is not a blanket instruction to maximise every booking. See ../prisma-campaign-budget-costings.md for the costing basis.
