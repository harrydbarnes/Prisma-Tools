# Submitted approval recipients

Draft feature guide for Ops Toolshed r1.9. Based on the reviewed source; live Prisma verification is still pending.

## What it does

Shows the email address(es) captured when the current user submits a campaign for approval.

## Find and use it

1. Open **Settings > Features** from the extension popup.
2. Search for **Submitted approval recipients** and select the matching option. Its default is **true**.
3. After you submit a campaign, look beside Submitted for captured recipient email addresses. This represents submissions captured for the current user, not a complete authoritative history of everyone’s approvals.
...
The option uses Chrome sync storage key `approverSubmittedRecipientDisplayEnabled`. Feature behaviour is implemented in `features/approver-pasting.js` and depends on the extension’s manifest script ordering. Campaign history, stats, remembered details and other local data have their own lifecycle; disabling an option is not equivalent to deleting its data.

## Documentation before this draft

README summary; Settings preview (322). This draft adds a standalone usage and troubleshooting page; brief UI descriptions and release notes are not complete operating instructions.

## Source references

- [settings.js:322](https://github.com/harrydbarnes/Ops-Toolshed-Extension/blob/b5a723e6d8f6acfeeb80e9a590eb57f12229b92d/settings.js#L322)
